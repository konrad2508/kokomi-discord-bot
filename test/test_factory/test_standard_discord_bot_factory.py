...
class StandardDiscordBotFactoryUnitTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.intents = patch('factory.standard_discord_bot_factory.Intents').start()
        self.intents_default_obj = self.intents.default.return_value
    ...
    def test_get_bot_returned_instance_received_correct_ctor_params(self) -> None:
        ...
        self.bot_ctor_mock.assert_called_once_with(command_prefix=self.cfg.prefix, help_command=None, intents=self.intents_default_obj)
        self.assertEqual(self.intents_default_obj.message_content, True)
