...
class PurgeCog(commands.Cog):
    '''Class representing the purge command. This command clears the queue and stops the currently playing song.'''
    ...
    def checker(func: Callable) -> Callable:
        '''Decorator checking whether the purge command can be run.
        ...
        @wraps(func)
        async def decorator(self: 'PurgeCog', ctx: commands.Context, *args):
    ...
    @checker
    async def purge_command(self, ctx: commands.Context, api: APIWrapperService = ...) -> None:
        '''Body of the command.'''

        server_id = api.get_server_id()

        self.music_player_service.purge(server_id)

        await self.embed_sender_service.send_success(ctx, Messages.PURGED_QUEUE)

def setup(bot: commands.Bot) -> None:
    bot.add_cog(PurgeCog(APIWrapperService, embed_sender_service, user_management_service, music_player_service))
